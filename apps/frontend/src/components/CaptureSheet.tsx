...
// Mobile one-tap capture: opening fires the camera input directly (no
// intermediate modal). The confirm sheet only appears once a photo exists.
export const CaptureSheet = forwardRef<CaptureSheetHandle, CaptureSheetProps>(({ plan }, ref) => {
  ...
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    releasePreview();
    setFile(picked);
    setPreviewUrl(typeof URL.createObjectURL === 'function' ? URL.createObjectURL(picked) : '');
  };
          ...
          <div className="fixed inset-0 z-[10000] bg-slate-900/70 backdrop-blur-sm flex items-end" onClick={uploading ? undefined : close}>
            ...
            >
              ...
              {previewUrl ? (
                ...
                <p className="text-sm font-bold text-slate-500 dark:text-slate-400 truncate mb-5">{file.name}</p>
              )}
