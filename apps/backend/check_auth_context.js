...
async function main() {
  const userId = "123"; // The ID I used for mock login
  ...
  reviewDocs.forEach(d => console.log(`- ${d.originalFileName} (${d.status})` ));
  ...
  recentDocs.forEach(d => console.log(`- ${d.originalFileName} (${d.status})` ));
}
