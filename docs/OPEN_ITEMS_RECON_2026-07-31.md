...
This file is a **dated snapshot, not a tracker.** Every coordinate below was re-derived against `25788bd`. It is
deliberately written to become *historical* rather than *stale*: when the state changes, write a new dated file.
The two documents in this repo that promised to stay current — `LAUNCH_TODO.md` and `DASHBOARD_REDESIGN_PROGRESS.md` —
are the only two that rotted. §7 records two items they still list as open that are in fact already closed.
...
**never been used**: zero issues across 130 pull requests.
...
| 3 — Class C English (counts) | `apps/frontend/tests/noHardcodedUserFacingText.test.ts:203-227` | Exact per-file census. See §3 for an arithmetic defect in that comment. |
...
`resetPasswordTooShort` is reset-flow copy and reusing it verbatim on signup would be wrong. The server still enforces
its own minimum, so this is a **UX defect, not a security hole**: the user learns the rule only after a round-trip.
...
| The only reachable DB is production | `README.md:69-73` |
...
|---|---|---|
| `apps/frontend/src/components/Sidebar.tsx:107` | `New Scan` (bare JSX text node) | Open. Named in **two** places already — `noHardcodedUserFacingText.test.ts:222` and `DASHBOARD_REDESIGN_PROGRESS.md:1370` — and fixed in neither. |
| `apps/frontend/src/screens/ProfileScreen.tsx:49` | `Security, notifications, and subscription management are under development.` | Open. Named at `noHardcodedUserFacingText.test.ts:223-224`. |
...
invisible — asserted at `noHardcodedUserFacingText.test.ts:19-21` and re-derived structurally here. The practical
...
`#1 "Create test.js"`, branch `tornidomaroc-web-patch-1`, open **118 days**. Unrelated to any audit; it is the only
open PR besides this one. It should be closed.

### 3.6 Arithmetic defect in the guard's census comment

`apps/frontend/tests/noHardcodedUserFacingText.test.ts:206-209` states 123 JSX-text violations of which "**115** sit in
five files", then lists: LandingScreen 51, DeleteAccountInfo 21, PrivacyPolicy 18, TermsOfService 14, RefundPolicy 12.
Those sum to **116**, not 115.
...
|---|---|---|
| Call sinks | **Yes** | `showToast`, `setError`, `setErrorMsg`, `setResetNotice` over `components/` + `screens/` + `contexts/` (`:31-46`) |
| Pin set | **Empty and green** | `KNOWN_PENDING_PR2: string[] = []` (`:74`) — audit #7 PR 2 landed and all pins were deleted in the same commit |
| Ratchet direction | **Two-way** | New literal fails (`:180`); stale pin also fails (`:191`) |
| **JSX text nodes** | **No — deliberately not built** | `:203-217`, because making it green today needs five whole-file exclusions plus a brand-token allowlist, which is the whitelist-driven shape §5 of the 2026-07-23 recon rejects as a trap |
...
## 6. BRANCH PROTECTION — ruleset 14939565 (correcting a false negative)
...
Verified at `GET /repos/tornidomaroc-web/scan-and-action/rulesets/14939565`:
...
`DASHBOARD_REDESIGN_PROGRESS.md:1335` still lists all five RTL sites as open, and `LAUNCH_TODO.md` still states
"~9 days remain (~Jul 4 2026)" — a deadline 27 days past at the time of writing, untouched since 2026-06-25. Both are
evidence for why this file is dated and immutable rather than maintained.
